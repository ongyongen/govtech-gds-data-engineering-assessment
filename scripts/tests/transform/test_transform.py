...
from .mocks import (
    ...
    mock_df_prepare_data_for_q2,
    mock_expected_df_prepare_data_for_q2
)
...
def test_prepare_data_for_q2():
    """
    Test that restaurant events dataframe is cleaned and ready for 
    export to csv
    """

    test_df = transform.prepare_data_for_q2(
        mock_df_prepare_data_for_q2
    )
    ...
    # Check that all required columns are present and values for the
    # respective data fields are correct
    for i in range(len(mock_expected_df_prepare_data_for_q2)):
        for column in mock_expected_df_prepare_data_for_q2.columns:
            assert test_df.loc[i, column] == \
                mock_expected_df_prepare_data_for_q2.loc[i, column]
